...
const BiddingDetails = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      // Pass the correct `contractId` to the API endpoint
      await apiConnector(
        "post",
  ...
  return (
    <div className="relative w-full h-full">
      ...
      {/* --- Details Card --- */}
      {selectedPothole && (
        <div className="absolute bottom-5 right-5 z-10 bg-white rounded-lg shadow-2xl p-4 w-full max-w-sm flex flex-col gap-3 animate-fade-in">{console.log(selectedPothole)}
          ...
          {/* ✨ FIX: Access the first item [0] in the contracts array to check the status */}
